...
IaEtfInfoMysql::IaEtfInfoMysql(const std::string& szIp, int nPort, const std::string& szUser, const std::string& szPassword)
{
    ...
    m_ti_db = new TiMysqlClient(szIp, nPort, szUser, szPassword, "ti_data");
};
...
void IaEtfInfoMysql::QueryEtfInfoList(int32_t date_num, std::vector<std::string>& fund_symbol_vec, std::vector<IaEtfInfo>& out)
{
    ...
    for (auto& row : result) {
        ...
        etfInfo.m_isCrossMarket = atoi(row["m_isCrossMarket"].c_str());
        etfInfo.m_hasRealityList = atoi(row["m_hasRealityList"].c_str());
...
void IaEtfInfoMysql::QueryFittedEtfList(int32_t date_num, std::vector<std::string>& fund_symbol_vec)
{
    std::vector<std::map<std::string, std::string>> result;
    char sql[512] = {0};
    sprintf(sql, "SELECT DISTINCT m_fundId FROM ti_fit_constituent_info WHERE m_tradeDate = %d;", date_num);
    m_ti_db->query(sql, result);
    for (auto& row : result) {
        fund_symbol_vec.push_back(row["m_fundId"]);
    }
};


void IaEtfInfoMysql::QueryFittedEtfInfoList(int32_t date_num, std::vector<std::string>& fund_symbol_vec, std::vector<IaEtfInfo>& out)
{
    std::vector<std::map<std::string, std::string>> result;
    char sql[256] = {0};
...
void IaEtfInfoMysql::QueryFittedEtfConstituentInfoList(int32_t date_num, std::vector<std::string>& fund_symbol_vec, std::vector<IaEtfConstituentInfo>& out)
{
    std::vector<std::map<std::string, std::string>> result;
    char sql[256] = {0};
    sprintf(sql, "SELECT * FROM ti_fit_constituent_info WHERE m_tradeDate = %d", date_num);
    std::string sql_str = sql;
    if (!fund_symbol_vec.empty()) {
        sql_str += " AND m_fundId IN (" + format_vec(fund_symbol_vec) + ");";
    }else{
    ...
    for (auto& row : result) {
        ...
        constituentInfo.m_redemption_amount = atof(row["m_redemption_amount"].c_str());
        constituentInfo.m_disclosure_vol = atof(row["m_disclosure_vol"].c_str());
        constituentInfo.m_reality_vol = atof(row["m_fit_reality_vol"].c_str());
        constituentInfo.m_cash_replaced_creation_premium_rate = atof(row["m_cash_replaced_creation_premium_rate"].c_str());
        constituentInfo.m_cash_replaced_redemption_discount_rate = atof(row["m_cash_replaced_redemption_discount_rate"].c_str());
        out.push_back(constituentInfo);
    }
