...
IaETFWorkerRedemptionEtf::IaETFWorkerRedemptionEtf(TiTraderClient* client, IaEtfQuoteDataCache* m_quote_cache, IaEtfSignalFactorPtr factor, std::string account)
...
void IaETFWorkerRedemptionEtf::OnRspOrderDelete(const TiRspOrderDelete* pData)
{
    
};

void IaETFWorkerRedemptionEtf::OnRtnOrderStatusEvent(const TiRtnOrderStatus* pData)
{
    if(isOver())
    {
        return;
...
void IaETFWorkerRedemptionEtf::OnTimer()
{
    if(isOver())
    {
        return;
...
void IaETFWorkerRedemptionEtf::updateExpectCost(TiQuoteSnapshotStockField* pData)
{
...
void IaETFWorkerRedemptionEtf::updateStatus()
{
    m_status.real_cost = 0;
    m_status.finish_volume = 0;
...
bool IaETFWorkerRedemptionEtf::hasQueueOrder()
{
    for (auto iter = m_order_map.begin(); iter != m_order_map.end(); iter++)
    {
...
int64_t IaETFWorkerRedemptionEtf::open()
{
    ...
    req.nTradeSideType = TI_TradeSideType_Redemption;
...
bool IaETFWorkerRedemptionEtf::isOver()
{
