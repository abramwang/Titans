...
IaETFWorkerSellEtf::IaETFWorkerSellEtf(int64_t id, TiTraderClient* client, IaEtfQuoteDataCache* m_quote_cache, IaEtfSignalFactorPtr factor, std::string account)
...
int64_t IaETFWorkerSellEtf::open()
{
    ...
    double price = IaEtfPriceTool::get_order_price(
            m_status.volume, etf_snap->bid_price, etf_snap->bid_volume, 5);
