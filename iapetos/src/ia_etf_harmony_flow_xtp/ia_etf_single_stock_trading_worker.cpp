...
int64_t IaETFSingleStockTradingWorker::open()
{
    ...
    if (m_side == TI_TradeSideType_Buy)
    {
        ...
        req.nOrderPrice = req.nOrderPrice > m_cache->m_last_snap.ask_price[5] ? req.nOrderPrice : m_cache->m_last_snap.ask_price[5] ;
    }
    if (m_side == TI_TradeSideType_Sell)
    {
        ...
        req.nOrderPrice = req.nOrderPrice < m_cache->m_last_snap.bid_price[5] ? req.nOrderPrice : m_cache->m_last_snap.bid_price[5] ;
    }
...
json IaETFSingleStockTradingWorker::getOrderArrayJson()
{
    json j = json::array();
    for (auto iter = m_order_map.begin(); iter != m_order_map.end(); iter++)
    {
