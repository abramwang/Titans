...
void IaETFSemiPilot::OnDepthOrderBookLevelRtn(const TiQuoteOrderBookField* pData)
{
    IaETFSemiTraderXtp::Locker locker(&m_commander->m_mutex);

    auto ob_iter = m_redeeming_observer_map.find(pData->symbol);
    if (ob_iter != m_redeeming_observer_map.end())
    {
        ob_iter->second->OnDepthOrderBookLevelRtn(pData);
    }
};
...
void IaETFSemiPilot::OnFactorRtn(const char* symbol, const char* factor_name, const json* pFactor, void* data)
{
    ...
    if (!m_init_time)
    {
        ...
        if ((datetime::get_now_timestamp_ms() - m_init_time) < 1000 * 60 * 1) //刚启动的1分钟内不交易
    ...
    auto ob_iter = m_redeeming_observer_map.find(symbol);
    if (ob_iter == m_redeeming_observer_map.end())
    {
        IaETFRedeemingObserver* observer = new IaETFRedeemingObserver(datetime::get_time_num(), (IaETFFactorOperator*)data, m_commander);
        m_redeeming_observer_map[symbol] = observer;
        m_commander->AddWorker(std::shared_ptr<IaETFTradingWorker>(observer));
    }else{
        ob_iter->second->OnFactorRtn(symbol, factor_name, pFactor, data);
    }
    ...
    */
    if (IaETFTradingSignalFilter::isRedeemingSignal(pOperator, j))
    {
        ...
        if ((cur_signal_time - pre_signal_time) / 100000 >= 3) 
        {
            ...
            if (!m_trade_switch)
            {
                double available_cash = m_commander->GetAvailableCash();
                double need_cash = pOperator->m_etf_last_snap.ask_price[0] * pOperator->m_minUnit;
                std::cout << "[isRedeemingSignal]" << need_cash << ", " << available_cash << "\n";
                if (need_cash > available_cash * 0.98) // 资金不足
                {
                    return;
                }else{
