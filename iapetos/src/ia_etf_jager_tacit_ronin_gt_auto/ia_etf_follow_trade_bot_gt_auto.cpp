...
IaEtfFollowTradeBotGtAuto::IaEtfFollowTradeBotGtAuto(uv_loop_s* loop, std::string configPath)
    : RedisCommander(loop)
{
    ...
    if(m_config){
        ...
        LOG(INFO) << "[IaEtfFollowTradeBotGtAuto] m_redis->connect flag: " << flag;
        resetStreamKey();
...
IaEtfFollowTradeBotGtAuto::~IaEtfFollowTradeBotGtAuto(){
    if(m_trade_client){
...
// 行情回调
////////////////////////////////////////////////////////////////////////
void IaEtfFollowTradeBotGtAuto::OnL2IndexSnapshotRtn(const TiQuoteSnapshotIndexField* pData)
{
    Locker locker(&m_mutex);
    m_quote_cache->OnL2IndexSnapshotRtn(pData);
};

void IaEtfFollowTradeBotGtAuto::OnL2FutureSnapshotRtn(const TiQuoteSnapshotFutureField* pData)
{
    Locker locker(&m_mutex);
    m_quote_cache->OnL2FutureSnapshotRtn(pData);
};

void IaEtfFollowTradeBotGtAuto::OnL2StockSnapshotRtn(const TiQuoteSnapshotStockField* pData)
{
    Locker locker(&m_mutex);
...
// 回调方法
////////////////////////////////////////////////////////////////////////
void IaEtfFollowTradeBotGtAuto::OnCommonJsonRespones(const json* rspData, int req_id, bool isLast, int err, const char* err_str)
{
    LOG(INFO) << "[IaEtfFollowTradeBotGtAuto::OnCommonJsonRespones]" << rspData;
    Locker locker(&m_mutex);
    if (err != 0)
    {
...
void IaEtfFollowTradeBotGtAuto::OnRspAccountInfo(const TiRspAccountInfo* pData)
{
    Locker locker(&m_mutex);
...
void IaEtfFollowTradeBotGtAuto::OnRspOrderDelete(const TiRspOrderDelete* pData)
{
    Locker locker(&m_mutex);
};

void IaEtfFollowTradeBotGtAuto::OnRspQryOrder(const TiRspQryOrder* pData, bool isLast)
{
    Locker locker(&m_mutex);
...
void IaEtfFollowTradeBotGtAuto::OnRspQryMatch(const TiRspQryMatch* pData, bool isLast)
{
    Locker locker(&m_mutex);
...
void IaEtfFollowTradeBotGtAuto::OnRspQryPosition(const TiRspQryPosition* pData, bool isLast)
{
    Locker locker(&m_mutex);
...
void IaEtfFollowTradeBotGtAuto::OnRtnOrderStatusEvent(const TiRtnOrderStatus* pData)
{
    Locker locker(&m_mutex);
...
void IaEtfFollowTradeBotGtAuto::OnRtnOrderMatchEvent(const TiRtnOrderMatch* pData)
{
    Locker locker(&m_mutex);
...
void IaEtfFollowTradeBotGtAuto::OnTimer()
{
    Locker locker(&m_mutex);
    ...
    if(!signal_out.empty())
    {
        try{
            ...
            //std::cout << "[IaEtfFollowTradeBotGtAuto::OnTimer] signal_size: " << signal_out.size() << std::endl;
        }catch(std::exception& e){
            std::cout << "[IaEtfFollowTradeBotGtAuto::OnTimer] " << e.what() << std::endl;
        }catch(...){
            std::cout << "[IaEtfFollowTradeBotGtAuto::OnTimer] " << "unknown exception" << std::endl;
        }
    }
    ...
    if ((time_num - m_query_time) > m_query_interval*1000)
    {
        ...
        std::cout << "[IaEtfFollowTradeBotGtAuto::OnTimer] QueryAsset QueryPositions: " << time_num << std::endl;
    }
...
void IaEtfFollowTradeBotGtAuto::OnCommandRtn(const char* type, const char* command)
{
    LOG(INFO) << "[IaEtfFollowTradeBotGtAuto::OnCommandRtn]" << type << ";" << command;
    std::cout << "[IaEtfFollowTradeBotGtAuto::OnCommandRtn]" << type << ";" << command;
...
void IaEtfFollowTradeBotGtAuto::onTimer(uv_timer_t* handle)
{
    IaEtfFollowTradeBotGtAuto* pThis = (IaEtfFollowTradeBotGtAuto*)handle->data;
    pThis->OnTimer();
};
...
/// @param err 
/// @param errStr 
void IaEtfFollowTradeBotGtAuto::onAuth(int err, const char* errStr){
    if(m_config){
...
void IaEtfFollowTradeBotGtAuto::onCommand(int err, std::vector<std::string> *rsp)
{
    std::cout << "onCommand:" << err << " " << rsp->size() <<  std::endl;
};

void IaEtfFollowTradeBotGtAuto::onXreadgroupMsg(const char* streamKey, const char* id, const char* type, const char* msg){
...
/// @brief 加载配置文件
/// @param iniFilePath 
int IaEtfFollowTradeBotGtAuto::loadConfig(std::string iniFileName){
    if(m_config){
        return -1;
    }
    ...
    m_config->szIp        = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["ip"]);
    m_config->nPort       = _iniFile["ia_etf_follow_trade_bot_gt_auto"]["port"];
    m_config->szAuth      = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["auth"]);

    m_config->nBlock          = _iniFile["ia_etf_follow_trade_bot_gt_auto"]["block"];
    m_config->szCommandStreamKey        = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["command_stream_key"]);
    m_config->szCommandStreamGroup      = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["command_stream_group"]);
    m_config->szCommandConsumerId       = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["command_consumer_id"]);
    
    m_config->szPositionKey      = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["position_key"]);
    m_config->szOrderKey         = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["order_key"]);
    m_config->szMatchKey         = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["match_key"]);

    m_config->szAccountKey       = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["account_key"]);

    m_config->szSignalMap       = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["signal_map"]);
    
    m_config->szSqlIp       = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["sql_ip"]);
    m_config->nSqlPort      = _iniFile["ia_etf_follow_trade_bot_gt_auto"]["sql_port"];
    m_config->szSqlUser     = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["sql_user"]);
    m_config->szSqlPassword = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["sql_password"]);

    m_config->szInfluxUrl   = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["influx_url"]);
    m_config->szInfluxToken = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["influx_token"]);
    m_config->szInfluxOrg   = string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["influx_org"]);
    m_config->szInfluxBucket= string(_iniFile["ia_etf_follow_trade_bot_gt_auto"]["influx_bucket"]);
...
void IaEtfFollowTradeBotGtAuto::resetStreamKey()
{
    if (!m_config)
    {
        return;
...
void IaEtfFollowTradeBotGtAuto::enterOrder(json &msg)
{
...
void IaEtfFollowTradeBotGtAuto::enterOrders(json &msg)
{
    if(!msg.is_array())
    {
       return;
...
void IaEtfFollowTradeBotGtAuto::cancelOrder(json &msg)
{
...
// 内部方法
////////////////////////////////////////////////////////////////////////
TiTraderClient* IaEtfFollowTradeBotGtAuto::GetTraderClient()
{
