...
void IaETFFactorOperator::parseConfig(IaETFFundInfo::FundInfo* info){
    ...
    {
        m_cashMap[i->m_symbol] = new IaETFConstituentCache(i->m_symbol.c_str(), i->m_exchange.c_str(), *i,
...
void IaETFFactorOperator::calcFactors(){
    ...
    double _ask_iopv = nAskIOPV;
    double _bid_iopv = nBidIOPV;
    double _high_limit_value = nHighLimitValue;
    double _low_limit_value = nLowLimitValue;
    double _diff = nDiff;
    double _purchasing = nPurchasingCost;
    double _redeeming = nRedeemingCost;
    datetime::get_format_timestamp_ms(_time, _iso_time, TI_TIME_STR_LEN);
    ...
    m_factor_json = {
        ...
        { "ask_iopv", _ask_iopv},
        { "bid_iopv", _bid_iopv},
        { "high_limit_value", _high_limit_value},
        { "low_limit_value", _low_limit_value},
...
void IaETFFactorOperator::OnL2StockSnapshotRtn(const TiQuoteSnapshotStockField* pData){
    ...
    if(iter != m_cashMap.end()){
        ...
        //800 毫秒计算一次
        if((pData->timestamp - m_constituent_timestamp) > 800)
        {
            calcFactors();
            calcTradingCost();
