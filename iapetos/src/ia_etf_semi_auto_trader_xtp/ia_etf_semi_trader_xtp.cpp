...
void IaETFSemiTraderXtp::OnCommonJsonRespones(const json* rspData, int req_id, bool isLast, int err, const char* err_str)
{
    ...
    {
        std::cout << "XtpOnQueryAsset: " << *rspData << std::endl;
...
int IaETFSemiTraderXtp::loadConfig(std::string iniFileName){
    ...
    m_config->szWorkerStatusStreamKey         = string(_iniFile["ia_etf_semi_trader_xtp"]["worker_status_key"]);
...
void IaETFSemiTraderXtp::resetStreamKey()
{
    ...
    if(!m_config->szWorkerStatusStreamKey.empty())
    {
        m_redis.del(m_config->szWorkerStatusStreamKey.c_str());
    }
};
...
void IaETFSemiTraderXtp::PublicWorkerStatus(json &status)
{
    return;
    if (!m_config)
    {
    ...
    m_redis.xadd(m_config->szWorkerStatusStreamKey.c_str(), status.dump().c_str());
};