...
TradeDemo::TradeDemo() : client_order_id_(0), req_id_(0), trader_api_(nullptr) { Init(); }
...
int TradeDemo::Login(const char *fund_acc) {
    // 交易登录，请将user替换成您的资金账户
    // 柜台通过客户机ip白名单进行登录鉴权控制
    // 密码字段使用EMC客户端时需输入，EMT_API当前不对密码进行校验
    auto session_id_ = trader_api_->Login("61.152.230.41", 19088, fund_acc, "", EMT_PROTOCOL_TCP);
    if (session_id_ == 0) {
...
void TradeDemo::Run() {
    ...
    auto session_id = session_map_["user1"];
...
void TradeDemo::OnOrderEvent(EMTOrderInfo *order_info, EMTRI *error_info, uint64_t session_id) {
    // 每次订单状态更新时，都会被调用
...
void TradeDemo::OnTradeEvent(EMTTradeReport *trade_info, uint64_t session_id) {
...
void TradeDemo::OnCancelOrderError(EMTOrderCancelInfo *cancel_info, EMTRI *error_info, uint64_t session_id) {
    // 撤单出错时会被调用
...
void TradeDemo::OnQueryOrderByPage(EMTQueryOrderRsp *order_info, int64_t req_count, int64_t order_sequence, int64_t query_reference, int request_id,
...
void TradeDemo::OnQueryTrade(EMTQueryTradeRsp *trade_info, EMTRI *error_info, int request_id, bool is_last, uint64_t session_id) {
...
void TradeDemo::OnQueryPositionByPage(EMTQueryStkPositionRsp *trade_info, int64_t req_count, int64_t trade_sequence, int64_t query_reference, int request_id,
                                      bool is_last, uint64_t session_id) {
