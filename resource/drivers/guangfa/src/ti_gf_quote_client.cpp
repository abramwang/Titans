...
TiGfQuoteClient::TiGfQuoteClient(std::string configPath, TiQuoteCallback* userCb)
{
    m_config = NULL;
    m_trading_day = 0;
...
void TiGfQuoteClient::OnLog(const int32_t& level, const char* log, uint32_t len)
{
    std::cout << "AMA log: " << "    level: " << level << "    log:   " << log << std::endl;
}

// 定义监控回调处理方法
void TiGfQuoteClient::OnIndicator(const char* indicator, uint32_t len)
{
    std::cout << "AMA indicator: " << indicator << std::endl;
}

// 定义事件回调处理方法  level 对照 EventLevel 数据类型 code 对照 EventCode 数据类型
void TiGfQuoteClient::OnEvent(uint32_t level, uint32_t code, const char* event_msg, uint32_t len)
{
...
// 定义快照数据回调处理方法
void TiGfQuoteClient::OnMDSnapshot(amd::ama::MDSnapshot* snapshot, uint32_t cnt)
{
...
//定义指数快照数据回调处理方法
void TiGfQuoteClient::OnMDIndexSnapshot(amd::ama::MDIndexSnapshot* snapshot, uint32_t cnt)
{
...
//定义逐笔委托数据回调处理方法
void TiGfQuoteClient::OnMDTickOrder(amd::ama::MDTickOrder* ticks, uint32_t cnt)
{
...
// 定义逐笔成交数据回调处理方法
void TiGfQuoteClient::OnMDTickExecution(amd::ama::MDTickExecution* tick, uint32_t cnt)
{
...
void TiGfQuoteClient::OnMDOrderBook(std::vector<amd::ama::MDOrderBook>& order_book)
{
...
int TiGfQuoteClient::loadConfig(std::string iniFileName){
    if(m_config){
...
void TiGfQuoteClient::init_ama_config()
{
    /*
        通道模式设置及各个通道说明:
...
void TiGfQuoteClient::subData(const char* exchangeName, char* codeList[], size_t len){
    /*
    按品种类型订阅信息设置:
...
bool TiGfQuoteClient::getETFCodeList(json &out)
{
    out = json::array();
...
bool TiGfQuoteClient::getCodeList()
{
