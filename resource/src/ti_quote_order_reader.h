#ifndef __TI_QUOTE_ORDER_READER_H__
#define __TI_QUOTE_ORDER_READER_H__
...
private:
    ...
    std::shared_ptr<arrow::Int64Array>     m_order_orino_array;    //
    std::shared_ptr<arrow::StringArray>    m_order_type_array;     //
...
public:
    ...
    int64_t GetTime();
    bool GetNext(TiQuoteOrderField* out);               // true: has next, false: is last;

    TiQuoteOrderReader(const char* path, char* field_list[], size_t len);
    virtual ~TiQuoteOrderReader(){};
};
