...
OcTraderCommanderGuangfa::OcTraderCommanderGuangfa(uv_loop_s* loop, std::string configPath)
    : RedisCommander(loop)
{
    m_client = new TiGfTraderClient(configPath, this);
    m_config = NULL;
    m_total_asset = 0;
    m_cash_asset = 0;
    ...
    if(m_config){
        ...
        LOG(INFO) << "[OcTraderCommanderGuangfa] flag: " << flag;
        resetStreamKey();
    }
    //*/
...
OcTraderCommanderGuangfa::~OcTraderCommanderGuangfa(){
    if(m_client){
        delete m_client;
        m_client = NULL;
    }
...
// 回调方法
////////////////////////////////////////////////////////////////////////
void OcTraderCommanderGuangfa::OnCommonJsonRespones(const json* rspData, int req_id, bool isLast, int err, const char* err_str)
{
    if (err != 0)
    {
...
void OcTraderCommanderGuangfa::OnRspOrderDelete(const TiRspOrderDelete* pData)
{
    Locker locker(&m_mutex);
};
void OcTraderCommanderGuangfa::OnRspQryOrder(const TiRspQryOrder* pData, bool isLast)
{
    /*
...
void OcTraderCommanderGuangfa::OnRspQryMatch(const TiRspQryMatch* pData, bool isLast)
{
    /*
    for (auto iter = m_workerList.begin(); iter != m_workerList.end(); iter++)
    {
...
void OcTraderCommanderGuangfa::OnRspQryPosition(const TiRspQryPosition* pData, bool isLast)
{
    /*
    for (auto iter = m_workerList.begin(); iter != m_workerList.end(); iter++)
    {
...
void OcTraderCommanderGuangfa::OnRtnOrderStatusEvent(const TiRtnOrderStatus* pData)
{
    Locker locker(&m_mutex);
    json j;
...
void OcTraderCommanderGuangfa::OnRtnOrderMatchEvent(const TiRtnOrderMatch* pData)
{
    Locker locker(&m_mutex);
    json j;
...
void OcTraderCommanderGuangfa::OnTimer()
{
    Locker locker(&m_mutex);
};


void OcTraderCommanderGuangfa::OnCommandRtn(const char* type, const char* command)
{
...
void OcTraderCommanderGuangfa::onTimer(uv_timer_t* handle)
{
    OcTraderCommanderGuangfa* pThis = (OcTraderCommanderGuangfa*)handle->data;
    pThis->OnTimer();
};
...
/// @param err 
/// @param errStr 
void OcTraderCommanderGuangfa::onAuth(int err, const char* errStr){
    if(m_config){
...
void OcTraderCommanderGuangfa::onCommand(int err, std::vector<std::string> *rsp)
{
    std::cout << "onCommand:" << err << " " << rsp->size() <<  std::endl;
};

void OcTraderCommanderGuangfa::onXreadgroupMsg(const char* streamKey, const char* id, const char* type, const char* msg){
...
/// @brief 加载配置文件
/// @param iniFilePath 
int OcTraderCommanderGuangfa::loadConfig(std::string iniFileName){
    if(m_config){
        return -1;
    }
    ...
    m_config->szIp        = string(_iniFile["oc_trader_commander_guangfa"]["ip"]);
    m_config->nPort       = _iniFile["oc_trader_commander_guangfa"]["port"];
    m_config->szAuth      = string(_iniFile["oc_trader_commander_guangfa"]["auth"]);

    m_config->nBlock          = _iniFile["oc_trader_commander_guangfa"]["block"];
    m_config->szCommandStreamKey     = string(_iniFile["oc_trader_commander_guangfa"]["command_stream_key"]);
    m_config->szCommandStreamGroup   = string(_iniFile["oc_trader_commander_guangfa"]["command_stream_group"]);
    m_config->szCommandConsumerId   = string(_iniFile["oc_trader_commander_guangfa"]["command_consumer_id"]);
    
    m_config->szOrderKey         = string(_iniFile["oc_trader_commander_guangfa"]["order_key"]);
    m_config->szMatchKey         = string(_iniFile["oc_trader_commander_guangfa"]["match_key"]);
...
void OcTraderCommanderGuangfa::resetStreamKey()
{
    if (!m_config)
    {
        return;
...
void OcTraderCommanderGuangfa::enterOrder(json &msg)
{
    TiReqOrderInsert req;
    memset(&req, 0, sizeof(TiReqOrderInsert));
...
// 内部方法
////////////////////////////////////////////////////////////////////////
TiTraderClient* OcTraderCommanderGuangfa::GetTraderClient()
{
    return m_client;
};