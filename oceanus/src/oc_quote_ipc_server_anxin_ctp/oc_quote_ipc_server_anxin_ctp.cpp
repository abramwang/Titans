#include <iostream>
#include <unistd.h>
#include "oc_quote_ipc_server_anxin_ctp.h"
#include "iniFile.h"
#include "datetime.h"
#include "ti_quote_formater.h"
#include <glog/logging.h>

OcQuoteIpcServerAnxinCtp::OcQuoteIpcServerAnxinCtp(uv_loop_s* loop, std::string configPath)
    : RedisCommander(loop)
{
    m_config = NULL;
    m_ctp_client = NULL;
    ...
    if(m_config){
        ...
        LOG(INFO) << "[OcQuoteIpcServerAnxinCtp] flag: " << flag;
        resetStreamKey();
...
OcQuoteIpcServerAnxinCtp::~OcQuoteIpcServerAnxinCtp(){
    if(m_config){
        delete m_config;
        m_config = NULL;
    }
...
void OcQuoteIpcServerAnxinCtp::OnTimer()
{
...
void OcQuoteIpcServerAnxinCtp::OnCommandRtn(const char* type, const char* command)
{
...
void OcQuoteIpcServerAnxinCtp::onTimer(uv_timer_t* handle)
{
    OcQuoteIpcServerAnxinCtp* pThis = (OcQuoteIpcServerAnxinCtp*)handle->data;
    pThis->OnTimer();
};
...
/// @param err 
/// @param errStr 
void OcQuoteIpcServerAnxinCtp::onAuth(int err, const char* errStr){
    if(m_config){
...
void OcQuoteIpcServerAnxinCtp::onCommand(int err, std::vector<std::string> *rsp)
{
    std::cout << "onCommand:" << err << " " << rsp->size() <<  std::endl;
};

void OcQuoteIpcServerAnxinCtp::onXreadgroupMsg(const char* streamKey, const char* id, const char* type, const char* msg){
...
/// @brief 加载配置文件
/// @param iniFilePath 
int OcQuoteIpcServerAnxinCtp::loadConfig(std::string iniFileName){
    if(m_config){
        return -1;
    }
...
void OcQuoteIpcServerAnxinCtp::resetStreamKey()
{
    if (!m_config)
    {
        return;
