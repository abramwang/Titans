...
public:
    ...
    int loadConfig(std::string iniFileName){
        ...
        m_config->szIp        = string(_iniFile["oc_quote_ipc_server_dongfang"]["ip"]);
        m_config->nPort       = _iniFile["oc_quote_ipc_server_dongfang"]["port"];
        m_config->szAuth      = string(_iniFile["oc_quote_ipc_server_dongfang"]["auth"]);

        m_config->m_sub_symbol_key      = string(_iniFile["oc_quote_ipc_server_dongfang"]["sub_symbol_key"]);
...
int main(int argc, char* argv[])
{
    ...
    TiDfQuoteClient api("./config.ini", cb);
    cb->client = &api;
